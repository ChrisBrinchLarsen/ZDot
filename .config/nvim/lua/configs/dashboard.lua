return{
    [1]="nvimdev/dashboard-nvim",      -- Requesting a plugin is assumed to be from a github repo so the syntax is "username/repository"
    ...
    dependencies = { "nvim-tree/nvim-web-devicons" },  -- A list of plugin names or plugin specs that should be loaded when the plugin loads. Dependencies are always lazy-loaded unless specified otherwise. When specifying a name, make sure the plugin spec has been defined somewhere else.
    ...
    event = "VimEnter",         -- Lazy-load on event. Events can be specified as BufEnter or with a pattern like BufEnter *.lua
